function highlightHTMLContent(htmlContent, plainText, plainTextPositions) {
  // Helper function to get the start and end positions of a given text in the htmlContent
  function getHtmlPositions(text) {
    const start = htmlContent.indexOf(text);
    const end = start + text.length;
    return { start, end };
  }

  // Sort the plainTextPositions array in descending order based on the start position
  plainTextPositions.sort((a, b) => b.start - a.start);

  // Loop through the plainTextPositions array and update the htmlContent by adding the <mark> tags
  for (const position of plainTextPositions) {
    const textToHighlight = plainText.substring(position.start, position.end);
    const { start, end } = getHtmlPositions(textToHighlight);

    const beforeHighlight = htmlContent.substring(0, start);
    const highlighted = `<mark>${htmlContent.substring(start, end)}</mark>`;
    const afterHighlight = htmlContent.substring(end);

    htmlContent = beforeHighlight + highlighted + afterHighlight;
  }
